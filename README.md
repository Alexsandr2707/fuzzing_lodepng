...
Для того чтобы скомпилировать проект, достаточно будет сделать 2 make в корневой папке проекта, на возможные Warnings не стоит обращать внимания)
AFL_HEADERS должна быть установлена и указывать на директорию с header файлами AFL++, путь должен быть ПРЯМЫМ, а НЕ ОТНОСИТЕЛЬНЫМ
...
Библиотека была выбрана достаточно низкоуровневая для получения большего простора манипуляций с png файлами.
 ...
 В начале каждого чанка есть его идентификатор, и информация о его структуре и размерах. В конце каждого чанка есть контрольная сумма. 
...
В этом можно убедится взглянув на результаты почти 3ех часового фаззинга (файл [2h_without_custom_mutator](docs/2_hours_without_custom_mutator_report/fuzzing_lodepng/utils/coverage/source/lodepng.cpp.func-sort-c.html)) в данных о покрытии кода. 

Попадания в необязательные чанки: cHRM, tIME, pHYs, sRGB, bKGD, tRNS, PLTE, sBIT, tEXt, iTXt, iCCP и т.д.:
...
[Полный список реализованных чанков](include/png_processing.h)
<br><br><br>

## О РЕАЛИЗАЦИИ МУТАТОРА


Сам по себе [мутатор](src/custom_mutators/afl_custom_mutator.c) имеет простую реализацию. Я использую только функцию afl_custom_fuzz, это связано с тем, что единственная "стоящая" мутация (которую я придумал) связана с формированием произвольного корректного png файла, остальные функции мутации не очень уместны. Содержимое чанка IDAT не столь важно и может быть (почти всегда) случайным, и только у этого чанка оно может быть совсем уж произвольным. Данные полученные от фаззера все (или частично) попадают в него. Остальные же чанки имеют достаточно четкую структуру и ограничения на содержимое, поэтому создаются в отдельной библиотеке.

### ДОБАВЛЕНЫ ДОПОЛНИТЕЛЬНЫЕ ОПЦИИ
Теперь мой мутатор не просто преобразует входные данные в png файл, но перед этим пытается их считать, как корректный png файл. Однако в связи с тем, что мутатор на практике принимает некорректную структуру, то оставлена возможность простого преобразования входных данных в png файл (в случае неудачного чтения). Если чтение удачное, то применяется следующие действия: случайным образом в png файл добавляются, удаляются и клонируются некоторые чанки. После чего файл запиывается в память и передается фаззеру.

Основная библиотека, реализующая добавление чанков случайным образом находится в [lib_png_random](src/lib_png_random)
<br><br><br>
...
### Библиотека [png_io](src/lib_png_io)
Реализует запись и чтение png файла из корректных приготовленных данных структуры png_processing_t в ее же поле - вектор png_out. А также отвечает за клонирование чанков.
<br><br>
...
1) Случайным образом выбираются дополнительные чанки (те что мы хотим добавить)
2) Вызывается функция png_config_chunks, которая формирует все данные чанков случайным образом, но сразу корректно
...
Стоит обратить внимание, что среди них есть один чанк, который не считается общепринятым: cSTM - custom chunk
...
В этой же директории лежат файлы [test.c](src/lib_png_random/test.c) и [infinity_testing.sh](src/lib_png_random/infinity_testing.sh) позволяющие наглядно протестировать работоспобность генерации случайных изображений (можно открыть выходное изображение и смотреть как оно меняется) при помощи реализованного мной вывода данных о сформированном изображении и утилиты pngcheck, подтверждающей корректность изображения.
...
Реализованно при помощи afl-cov (См. [coverage](utils/coverage)). <br>
Возможно необходима дополнительная настройка путей после клонирования... <br>
Полученные данные покрытий, как было озвучено ранее, находятся в директориях [С МУТАТОРОМ](docs/1_hour_custom_fuzzing_report/web) и [БЕЗ МУТАТОРА](docs/2_hours_without_custom_mutator_report)
<br><br>

### Минимизация и сжатие корпуса
Реализовано при помощи afl-cmin и afl-tmin (См. [minimize_corpus](utils/minimize_corpus)).<br>
Возможно необходима дополнительная настройка путей после клонирования... <br>

При этом сначала выполняется копирование всех файлов корпуса (из всех фаззеров), потом отсекаются бесполезные файлы при помощи afl-cmin и частично сжимаются при помощи afl-tmin. Мое сжатие получилось незначительным (около 2-3%) скорее всего это связано со сложностью сжатия png файлов, сложностью их структуры. <br>
(При необходимости я могу выложить получившийся корпус в этот проект)
<br><br>
...
Увы от того запуска остался только 1 скриншот, при необходимости могу добавить еще <br>
...
### Скриншоты демонстрирующие одновременную работу нескольких фаззеров
![many_fuzzers](docs/1_hour_custom_fuzzing_report/pics/many_fuzzers.jpg)
<br><br>

### Работа усовершенствованного фаззера 
...
Сразу же заметен прирост количества линий и функций. Однако это только первый взгляд на результаты. При детальном рассмотрении все становится еще лучше!
...
Виден огромный прирост в количестве их вызов, а следственно можно утверждать, что фаззер лучше справляется с тестированием программы. Больше попаданий == программа лучше протестирована. 

Аналогичные результаты можно наблюдать в функциях, отвечающих за чтение и анализ основных чанков. Результаты показывают, что попадание даже в основные чанки возросло не менее чем в 2 раза (это функции чтения info и idat). <br><br><br>
...
Можно говорить, что мутатор хорошо справляется со своей задачей. Наверняка его можно еще немного улучшить добавив случайный порядок записи чанков (достаточно сложно, необходимо углубиться в документацию о чанках и переписать большое количество кода) или например добавить больше поддерживаемых языков в чанке iTXt (это задача чисто механическая и нетрудно выполнимая).
